...
copy_shared_pages(envid_t child)
{
	...
	uint64_t va = UTEXT;
	uint64_t end = UTOP ;
	int perm = (PTE_AVAIL | PTE_P | PTE_U | PTE_W);
	int ret;
	while(va < end){
		if(uvpml4e[VPML4E(va)] && uvpde[VPDPE(va)]){
				if(uvpd[VPD(va)]){
					if(uvpt[VPN(va)] & PTE_SHARE){
						ret = sys_page_map(thisenv->env_id, (void *)va, child, (void *)va, ((uvpt[VPN(va)]) & (perm)));
						if(ret < 0)
							panic("Panic in copy shared pages");
					}
					va = va + PGSIZE;
				}else{
					va = va + (512*PGSIZE);
