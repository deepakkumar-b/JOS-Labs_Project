...
serve_read(envid_t envid, union Fsipc *ipc)
{
	...
	size_t readbyte = req->req_n;
	int ret2;
	struct OpenFile *file;
	ssize_t ret3;
	if(readbyte > PGSIZE)
		readbyte = PGSIZE;
	ret2 = openfile_lookup(envid, req->req_fileid, &file);
	if(ret2 < 0)
		return ret2;
	ret3 = file_read(file->o_file, (void *)ret->ret_buf, readbyte, file->o_fd->fd_offset);
	if(ret3 < 0)
		return (int)ret3;
	file->o_fd->fd_offset = file->o_fd->fd_offset + ret3;
	return ret3;
