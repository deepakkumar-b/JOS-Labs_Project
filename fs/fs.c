...
file_block_walk(struct File *f, uint32_t filebno, uint32_t **ppdiskbno, bool alloc)
{
	...
	if(filebno >= (NDIRECT + NINDIRECT)){
		cprintf("Filebno is out of range \n");
		return -E_INVAL;
	}else if(filebno < NDIRECT){
		*ppdiskbno = &f->f_direct[filebno];
		return 0;
	} else if(!(f->f_indirect) && !(alloc)){
		cprintf("Indirect block allocation was needed \n");
		return -E_NOT_FOUND;
//	}else if(!(f->f_indirect)){
		//Remaining to allocate block. Minesh
		//this part seems irrelevant for read-only file-system
	}
...
file_get_block(struct File *f, uint32_t filebno, char **blk)
{
	...
	ret = file_block_walk(f, filebno, &tmp, false);
	if(ret < 0)
		return -E_INVAL;
	*blk= diskaddr(*tmp);
	return 0;
