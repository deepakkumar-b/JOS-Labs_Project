...
#include <kern/module.h>	//minesh lab7
...
//      cprintf("nextfree %x\n",nextfree);
	if(npages * PGSIZE < PADDR(nextfree))
		panic("No more physical memory left to allocate 0x%0x\n",nextfree);
        return result;
...
//	envs =(struct Env *)boot_alloc(NENV * sizeof(struct Env));
	...
	//Lab7 Minesh
	modules_lkm = (struct LKM_Module *)boot_alloc(MAX_LKM * sizeof(struct LKM_Module));
	//lab7 code end
	page_init();
	//check_page_alloc();
	//page_check();
	...
	// Your code goes here:
	boot_map_region(boot_pml4e, UPAGES, PTSIZE, PADDR(pages), PTE_U | PTE_P); //initially it was PTE_U | PTE_P instead of just PTE_U
...
//	boot_map_region(boot_pml4e, UPAGES, n, PADDR(pages), PTE_U);
	...
	// LAB 3: Your code here.
        ...
        //////////////////////////////////////////////////////////////////////
	boot_map_region(boot_pml4e, UENVS, PTSIZE, PADDR(envs), (PTE_U | PTE_P));	//initially it was PTE_U | PTE_P instead of just PTE_U
//	n = ROUNDUP(NENV * sizeof(struct Env), PGSIZE);
//	boot_map_region(boot_pml4e, UENVS, n, PADDR(envs), PTE_U);
	...
	// Your code goes here:
//	boot_map_region(boot_pml4e, KSTACKTOP-KSTKSIZE, KSTKSIZE, PADDR(bootstack), PTE_W | PTE_P);	//from lab2 now removed
	//////////////////////////////////////////////////////////////////////
	...
	// Permissions: kernel RW, user NONE
	// Your code goes here: 
	boot_map_region(boot_pml4e, KERNBASE, (npages * PGSIZE), 0x0, (PTE_W | PTE_P));	//initially it was PTE_W | PTE_P instead of just PTE_W
	// Check that the initial page directory has been set up correctly.
	// Initialize the SMP-related parts of the memory map
	mem_init_mp();
//	check_page_free_list(1); //added minesh
//	cprintf("After check page free list \n"); //added minesh
...
mem_init_mp(void)
{
	...
	int i = 0;
	uintptr_t kstacktop_i,tmp = KSTKSIZE + KSTKGAP;
	while(i < NCPU)
	{
		kstacktop_i = KSTACKTOP - (i * tmp);
...
/*	for (i = 0; i < npages; i++) {
	...
	page_free_list= NULL;
	for (i = 1; i < npages; i++) {                  //start of actual code
		pages[i].pp_ref = 0;
		if(i >= BOOT_PAGE_TABLE_START / PGSIZE && i <= BOOT_PAGE_TABLE_END / PGSIZE)
			pages[i].pp_ref = 1;
	...
	pages[0].pp_ref++;
	j=PADDR(boot_alloc(0))/PGSIZE;
	for (i = IOPHYSMEM / PGSIZE;i < j; i++){
...
{
//      cprintf("pml4e=%x va=%x \n",pml4e,va);
//      return NULL;
	pte_t *pte;
        pml4e_t *pml2;
        struct PageInfo *pg;
        pml2=&pml4e[PML4(va)];
//      cprintf("pml4e pml2=%x\n",pml2);
        pdpe_t *pdpe=(pdpe_t *)(PTE_ADDR(*pml2));
        if(pdpe == 0){
                if(create == false)
                        return NULL;
                else{
...
//                      cprintf("pml4e pg=%x\n",*pg);
                        if(pg == NULL)
                                return NULL;
//                      pml4e[PML4(va)]=page2pa(pg) | PTE_U | PTE_W | PTE_P;

                        pg->pp_ref++;
                        pdpe=(pdpe_t *) page2kva(pg);
                        pte=pdpe_walk(pdpe,va,create);
                        if(pte == NULL){
                                page_decref(pg);
                        	return NULL;
			}else{
                                *pml2 = (page2pa(pg)) | PTE_P | PTE_W | PTE_U;
                                return pte;
                        }
                }
        }
        else{
                pdpe = (pdpe_t *) KADDR(PTE_ADDR(*pml2));
                pte = pdpe_walk(pdpe,va,create); 
                return pte;
        }
        return NULL;  
...
pdpe_walk(pdpe_t *pdpe,const void *va,int create){
	...
	struct PageInfo *pg;
	pdpe2=&pdpe[PDPE(va)];
	pde_t *pde=(pde_t *)(PTE_ADDR(*pdpe2));
        if(pde == 0){
                if(create == false)
                        return NULL;
                else{
...
//              	cprintf("pdpe pg=%x\n",*pg);
                        if(pg == NULL)
                                return NULL;
                        pg->pp_ref++;
                        pde=(pde_t *) page2kva(pg);
                        pte=pgdir_walk(pde,va,create);
                        if(pte == NULL){
                                page_decref(pg);
                        	return NULL;
			}else{
                                *pdpe2 = (page2pa(pg)) | PTE_P | PTE_W | PTE_U;
//      			cprintf("pdpe pte=%x\n",pte);
			        return pte;
                        }
                }
                ...
                pde = (pde_t *) KADDR(PTE_ADDR(*pdpe2));
                pte = pgdir_walk(pde,va,create);
	//	cprintf("pdpe pte=%x\n",pte); //deepak - was commented
                return pte;
        }
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	struct PageInfo* pg;
        ...
        if (*pde & PTE_P) {
                ...
                pg = page_alloc(ALLOC_ZERO);
                if (pg == NULL) {
//                      cprintf("PGDIR Returning NULL\n");
                        return NULL;
                }
                pg->pp_ref++;
                pte = (pte_t *) page2kva(pg);
                memset(pte, '\0', PGSIZE);
                *pde = page2pa(pg) | PTE_P | PTE_W | PTE_U;
        }
//      cprintf("pgdir pte=%x\n",pte);
        return &pte[PTX(va)];
...
boot_map_region(pml4e_t *pml4e, uintptr_t la, size_t size, physaddr_t pa, int perm)
{
	...
	for(i = 0; i <  size; i += PGSIZE){
		pte_t *pte = pml4e_walk(pml4e, (void *) la, true);	//instead of perm it was true
		*pte = pa | perm | PTE_P;
		la += PGSIZE;
		pa += PGSIZE;
	}
...
page_remove(pml4e_t *pml4e, void *va)
{
	...
	pte_t *pte;
	struct PageInfo *pg;
	pg = page_lookup(pml4e, va, &pte); //mark the use of &pte as last arg
	if(pg != NULL){
//              cprintf("page remove called\n");
		*pte = 0;
...
//	panic("mmio_map_region not implemented");
	...
	if(base + size > MMIOLIM){
		panic("Kernel Panic in mmio_map_region \n");
	}else{
		boot_map_region(boot_pml4e, base, size, pa, (PTE_W | PTE_PCD | PTE_PWT | PTE_P));
		base = base + size;
	}
	return (void *)result;
}
...
{
	// LAB 3: Your code here.
	uintptr_t j,i = (uintptr_t)va;
	pte_t *pte;
//	cprintf("Entering user_mem_check, va=%016x len=%016x i=%016x \n",va,len,i);
	if((uint64_t)i >= ULIM)
	{
	//	cprintf("Entering if of ULIM \n");
		user_mem_check_addr = i;
		return -E_FAULT;
	}
	while(i < (uintptr_t)(va + len))
	{
/*		if((uint64_t)i >= ULIM)
        	{
			cprintf("Entering if of ULIM \n");
	...
	//	cprintf("Entering user_mem_asser. BEfore pml4e\n");
		pte = pml4e_walk(env->env_pml4e, (const void *)i, 0);
//		cprintf("Entering user_mem_asser. After pml4e %016x \n",pml4e_walk(env->env_pml4e, (const void *)i, 0));
		if((!pte) || (!(*pte & (perm | PTE_P))))
		{
//			cprintf("Inside if of pte \n");
			user_mem_check_addr = i;
			return -E_FAULT;
		}
//		cprintf("After if of pte \n");
		j = (uintptr_t)ROUNDUP(i, PGSIZE);
		if(i != j){
			i = j;
		}else{
			i = i + PGSIZE ;
...
//	cprintf("leaving user_mem_check\n");
	return 0;
...
{
//	cprintf("Entering user_mem_asser\n");
	if (user_mem_check(env, va, len, perm | PTE_U) < 0) {
		cprintf("[%08x] user_mem_check assertion failure for "
			"va %08x\n", env->env_id, user_mem_check_addr);
		env_destroy(env);	// may not return
	}
//	cprintf("Leaving User_mem_Assert\n");
}
