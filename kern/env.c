...
//	cprintf("region_alloc e=%x pml4e = %x va=%x len=%x \n",e,e->env_pml4e,va,len);
	size_t i;
	int j=1;
	struct PageInfo *pg;
//	va = ROUNDDOWN(va, PGSIZE);
	...
	for(i = 0; i < len; i += PGSIZE){
		...
		if(pg == NULL){
			...
			j = page_insert(e->env_pml4e, pg, va+i, (PTE_U | PTE_W));
			if(j != 0){
...
//	cprintf("\nEntering function\n");
	ehdr = (struct Elf *)(binary);
	e->elf = binary;
	if(ehdr->e_magic != ELF_MAGIC)
	{
		panic("ELF header not valid!!\n");
		return;
	}

	lcr3(e->env_cr3);
	
	ph1 = (struct Proghdr *)((uint8_t *)ehdr + ehdr->e_phoff);
	ph2 = ph1 + ehdr->e_phnum;
	
	for(; ph1 < ph2; ph1++)
	{
		if(ph1->p_type == ELF_PROG_LOAD)
		{
			if(ph1->p_filesz > ph1->p_memsz)
			{
			...
			//walk =(pte_t *)pml4e_walk(e->env_pml4e,(void *)ph1->p_va, 0);
			//pg = page2kva(walk);
			//cprintf("walk value : %x", walk);
			memcpy((void *)ph1->p_va, binary + ph1->p_offset, ph1->p_filesz);
	...
	pg = page_alloc(0);
	if(pg != NULL){
		err = page_insert(e->env_pml4e, pg, (void *)USTACKTOP - PGSIZE, PTE_U | PTE_W);
        	if(err == -E_NO_MEM)
        	{
                	panic("Page insert error in load icode  %e\n", err);
                	return;
        	}
	}else{
		panic("page alloc error in load icode %e\n", -E_NO_MEM);
                return;
	}
	e->env_tf.tf_rip = ehdr->e_entry;
//    	e->elf = binary;
}
...
env_create(uint8_t *binary, enum EnvType type)
{
	...
	struct Env *env2;
	int i = env_alloc(&env2, 0);
	if(i < 0){
		panic("Env_alloc error: %e", i);
	}
	if(type == ENV_TYPE_FS){
		env2->env_tf.tf_eflags =  env2->env_tf.tf_eflags | FL_IOPL_3;
	}
	load_icode(env2, binary);
	env2->env_type = type;
}
...
env_run(struct Env *e)
{
	...
	//	e->env_tf to sensible values.
		...
		// LAB 3: Your code here.
//	cprintf("Entering env_run for e=%x\n",e);
...
//	cprintf("line 611 \n");
	curenv->env_runs++;
//	cprintf("line 613 \n");
	unlock_kernel(); //minesh
//	cprintf("line 615 \n");
	lcr3(curenv->env_cr3);
//	cprintf("line 617 \n");
	env_pop_tf(&curenv->env_tf);
//	cprintf("Leaving env_run for e=%x\n",e);
